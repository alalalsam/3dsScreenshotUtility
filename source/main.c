// made by alalalsam. copypasted from Luma3DS/sysmodules/rosalina/source/main.c
...
int main(void)
{
    ...
    Draw_Init();
    MyThread *screenshotThread = screenshotCreateThread();

    if (R_FAILED(ServiceManager_Run(services, notifications, NULL)))
        svcBreak(USERBREAK_PANIC);

    MyThread_Join(screenshotThread, -1LL);
    return 0;
}